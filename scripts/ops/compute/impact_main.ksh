...
do
    case $var in
        ...
        o) mode=$OPTARG; case $OPTARG in
               (txt)  ;;
               (db)   include=impact_compute; py_storer=dbstorer; py_store=im_ops;;
               (t2db) include=impact_load; py_store=stats.txt; py_database=im_ops;;
               (*)    usage;;
           esac
    esac
done
...
export SEMPERPY_VERSION=ops
. $root/semperpy.source
