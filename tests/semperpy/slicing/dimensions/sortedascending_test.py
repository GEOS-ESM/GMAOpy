...
class SortedAscendingTest(unittest.TestCase):
    ...
    def setUp(self):
        self.dim_ = SortedAscendingDimension(self.name,self.official,self.variable)
    ...
    def test_slicing(self):
        ...
        s = self.dim_.findSlice({12:17})
        self.assertEqual(s,[slice(0, 6, 1)])
        with self.assertRaises(IndexError):
            s = self.dim_.findSlice([12,15,25,20])

if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(SortedAscendingTest)
